...
public class GameServer implements Runnable{
	...
	/**
	 * Constructor starts up the main.server
	 * 
	 * @param 	port
	 * @param	ui
	 */
	...
	private class ClientHandler extends Thread{
				...
				try{
					...
					if(object instanceof String){
						...
						if(sInput.equals("ENDTURN")){
							...
							main.client.Character tempChar = characterMap.get((String)input.readObject());
		...
		/**
		 * Sends a out a message to the main.client that it has timed out
		 */
		...
		public void clientsTurn(boolean enableButtons){
			...
			while(characterMap.get(clientMapid.get(counter)).sleeping() != 0){
				...
				main.client.Character charr = characterMap.get(clientMapid.get(counter));
				charr.passATurn();
				try {
		...
		public synchronized void createCharacter(String name) {
			...
			main.client.Character myCharacter = new main.client.Character(name, -1, -1);
		...
		public void updateCharPos(main.client.Character charr) {
