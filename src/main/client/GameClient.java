...
public class GameClient implements Serializable{
	...
	private Tile[][] map;
	private HashMap<String, main.client.Character> characterMap = new HashMap<String, main.client.Character>();
	...
	/**
	 * Creates a new class "Connection" with will connect to the main.server
	 * 
	 * @param serverIp
	 * @param port
	 */
	...
	/**
	* Method that disconnects the main.client from the main.server
	*/
	...
	/**
	* Inner class that handles the connection between main.client and main.server
	*/
	...
	private class Connection extends Thread{
			...
			while(!Thread.interrupted()){
				try{
					...
					if(object instanceof Integer){
						...
						main.client.Character character = (main.client.Character)input.readObject();
					...
					else if (object instanceof main.client.Character){
						main.client.Character character = (main.client.Character) object;
						character.passATurn();
						if(character.sleeping() > 0){
							for(ViewerListener listener: listeners){
								listener.setIconSleep(character.getCharacterName(), false);
		...
		/**
		 * Communicates the winner of the game to the main.server
		 */
		...
		/**
		 * Sends the username to the main.server
		 * 
		 * @param character
		 */
		...
		/**
		 * Sends the character how got shot to the main.server
		 * 
		 * @param target
		 */
		...
		/**
		 * Sends a character to the main.server
		 * 
		 * @param character
		 */
		
		public void flushCharacter(main.client.Character character){
			try{
				output.writeObject(character);
				output.flush();
			} catch (IOException e) {
		...
		/**
		 * Checks the direction the character wants to move an if its a valid move tells the main.server
		 * 
		 * @param character
		 * @param direction
		 */
		
		public void moveChar(main.client.Character character, String direction) {
		...
		/**
		 * Tells the main.server to show the treasure
		 * 
		 * @param status
		 */
		...
		public void updateCharacter(main.client.Character character){
	...
	public boolean checkMove(main.client.Character me, String dir) {
		boolean ret = false;
		switch (dir) {
		case "LEFT":
			if(map[me.getRow()][me.getCol() - 1].getAccessible()){
...
//		map[16][0].setNext(0, -1);
	 ...
	 * 
	 * @param character
	 * @return ArrayList<main.client.Character>
	 */
	
	public ArrayList<main.client.Character> lookingForAShot(main.client.Character character){
		ArrayList<main.client.Character> charArray = new ArrayList<>();
		int charInt = 0;
